...
if zeta >= 1
    aux_p        = sqrt(zeta^2 - 1);
    s1_p         = -omega_n * (zeta - aux_p);
    s2_p         = -omega_n * (zeta + aux_p);
    tau_slow_p   = -1 / s1_p;
    tau_fast_p   = -1 / s2_p;
else
    tau_slow_p   = 1 / (zeta * omega_n);
    tau_fast_p   = tau_slow_p;
end
tau_crit_p     = 1 / omega_n;
delay_p        = tau_slow_p - tau_crit_p;
...
%% ============================================================
%  FIGURE 1 — Ride Analysis
%% ============================================================
col_front = [0.00  0.45  0.85];
col_rear  = [0.85  0.25  0.05];
col_cg    = [0.10  0.60  0.20];
col_pitch = [0.65  0.10  0.75];
col_road  = [0.55  0.55  0.10];
col_grid  = [0.75  0.75  0.75];
col_txt   = [0.10  0.10  0.10];
...
yline(0,'-','Color',[0 0 0 0.15],'LineWidth',0.8);
...
legend('Location','northeast','TextColor',col_txt,'Color','w','EdgeColor',col_grid,'FontSize',8);
...
legend('Pitch \theta(t)','Location','northeast', ...
       'TextColor',col_txt,'Color','w','EdgeColor',col_grid);
...
plot(z_front_mm(1),  z_rear_mm(1),  'o','MarkerSize',8,'MarkerFaceColor',[0.2 1 0.4],'MarkerEdgeColor','k','LineWidth',1.2,'DisplayName','Start Point');
plot(z_front_mm(end),z_rear_mm(end),'s','MarkerSize',8,'MarkerFaceColor',[1 0.3 0.3],'MarkerEdgeColor','k','LineWidth',1.2,'DisplayName','End Point');
ax_lim = max(abs([z_front_mm z_rear_mm]))*1.1;
plot([-ax_lim ax_lim],[-ax_lim ax_lim],'--','Color',[0 0 0 0.15],'LineWidth',0.8,'DisplayName','Unity Line');
...
legend('Trajectory','Start Point','End Point','Location','northwest', ...
       'TextColor',col_txt,'Color','w','EdgeColor',col_grid,'FontSize',8);
...
annotation('textbox',[0 0.97 1 0.03], ...
    ...
    'Color',[0.20 0.20 0.20],'FontSize',9.5, ...
...
%% ============================================================
col_crit = [0.80  0.60  0.00];
col_ap   = [0.00  0.45  0.75];
col_ab   = [0.10  0.65  0.20];
col_brk  = [0.85  0.30  0.00];
bg_lbl   = [0.95  0.95  0.95];
...
plot(tp, ypc, '--', 'Color', col_crit, 'LineWidth', 2.0, ...
     'DisplayName', sprintf('\\zeta = 1   (1/\\omega_n = %.4f s)', tau_crit_p));
plot(tp, ypa, '-',  'Color', col_ap,   'LineWidth', 2.5, ...
     'DisplayName', sprintf('\\zeta = %.4f  (\\tau_{slow} = %.4f s)', zeta, tau_slow_p));

yline(0.632, ':', 'Color', [col_txt 0.35], 'LineWidth', 1.0, 'HandleVisibility', 'off');
yline(1.0,   '-', 'Color', [col_txt 0.12], 'LineWidth', 0.8, 'HandleVisibility', 'off');

text(tp(1), 0.632, '  63.2%', 'Color', col_txt, 'FontSize', 9, ...
     'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');

if ~isnan(tau63_pitch_crit)
    xline(tau63_pitch_crit,  '--', 'Color', [col_crit 0.70], 'LineWidth', 1.2, 'HandleVisibility', 'off');
end
if ~isnan(tau63_pitch_actual)
    xline(tau63_pitch_actual,'--', 'Color', [col_ap   0.70], 'LineWidth', 1.2, 'HandleVisibility', 'off');
end
xline(settle_crit_p, ':', 'Color', [col_crit 0.45], 'LineWidth', 1.0, 'HandleVisibility', 'off');
xline(settle_p,      ':', 'Color', [col_ap   0.45], 'LineWidth', 1.0, 'HandleVisibility', 'off');
...
text(settle_crit_p, -0.090, sprintf('t_{s,crit}\n%.4f s', settle_crit_p), ...
     'Color', col_crit, 'FontSize', 8, 'HorizontalAlignment', 'center', ...
     'VerticalAlignment', 'top', 'BackgroundColor', bg_lbl, 'Margin', 1);
text(settle_p, -0.090, sprintf('t_{s,act}\n%.4f s', settle_p), ...
     'Color', col_ap, 'FontSize', 8, 'HorizontalAlignment', 'center', ...
     'VerticalAlignment', 'top', 'BackgroundColor', bg_lbl, 'Margin', 1);

if ~isnan(tau63_pitch_crit) && ~isnan(tau63_pitch_actual)
    drawBracket(axA, tau63_pitch_crit, tau63_pitch_actual, 0.50, 0.022, ...
        sprintf('\\Delta\\tau_{63} = %.4f s', tau63_pitch_actual - tau63_pitch_crit), col_brk, 10);
end
drawBracket(axA, settle_crit_p, settle_p, 1.10, 0.022, ...
    sprintf('\\Delta t_{settle} = %+.4f s', delta_settle_p), col_brk, 10);
...
legend('Location', 'southeast', 'TextColor', col_txt, ...
       'Color', 'w', 'EdgeColor', col_grid, 'FontSize', 10);
ylim([-0.16 1.20]);
...
plot(tb, ybc, '--', 'Color', col_crit, 'LineWidth', 2.0, ...
     'DisplayName', sprintf('\\zeta = 1   (1/\\omega_n = %.4f s)', tau_crit_b));
...
if ~isnan(tau63_bounce_crit)
    xline(tau63_bounce_crit,  '--', 'Color', [col_crit 0.70], 'LineWidth', 1.2, 'HandleVisibility', 'off');
end
if ~isnan(tau63_bounce_actual)
    xline(tau63_bounce_actual,'--', 'Color', [col_ab   0.70], 'LineWidth', 1.2, 'HandleVisibility', 'off');
end
xline(settle_crit_b, ':', 'Color', [col_crit 0.45], 'LineWidth', 1.0, 'HandleVisibility', 'off');
xline(settle_b,      ':', 'Color', [col_ab   0.45], 'LineWidth', 1.0, 'HandleVisibility', 'off');
...
text(settle_b, -0.090, sprintf('t_{s,act}\n%.4f s', settle_b), ...
     'Color', col_ab, 'FontSize', 8, 'HorizontalAlignment', 'center', ...
     'VerticalAlignment', 'top', 'BackgroundColor', bg_lbl, 'Margin', 1);

if ~isnan(tau63_bounce_crit) && ~isnan(tau63_bounce_actual)
    drawBracket(axB, tau63_bounce_crit, tau63_bounce_actual, 0.50, 0.022, ...
        sprintf('\\Delta\\tau_{63} = %.4f s', tau63_bounce_actual - tau63_bounce_crit), col_brk, 10);
end
drawBracket(axB, settle_crit_b, settle_b, 1.10, 0.022, ...
    sprintf('\\Delta t_{settle} = %+.4f s', delta_settle_b), col_brk, 10);
...
function drawBracket(ax, x1, x2, y, tickH, label, col, fs)
    ...
    text(ax, mean([x1 x2]), y + tickH*1.8, label, ...
         ...
         'BackgroundColor', [0.95 0.95 0.95], 'Margin', 2);
end
